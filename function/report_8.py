...
def write_8_docx(self,param):
    # 生成实验报告
    document = Document('./resources/report/实验八 吸收-解吸实验.docx')

    document.add_paragraph('实验日期' + param.date + "\t填料层高度" + param.height +
                           "\t塔内径" + param.diameter +
                           "\t大气压" + param.pressure + "\t室温" + param.temp +
                           "\t流量计标定状态:T=" + param.T + " P=" + param.P)

    document.add_paragraph("                表1 填料塔流体力学性能")
    table1 = document.add_table(len(param.data) + 1, len(param.data[0]), style='Table Grid')
    ...
    for i in range(len(param.data)):
        for j in range(len(param.data[0])):
            table1.rows[i + 1].cells[j].text = str(param.data[i][j])

    document.add_paragraph("                表2 CO₂吸收传质系数测定原始数据记录表")
    table3 = document.add_table(len(param.data3[0]), 3, style='Table Grid')
    ...
    for i in range(len(param.data3)):
        for j in range(len(param.data3[0])):
            table3.rows[j].cells[1 + i].text = str(param.data3[i][j])
    document.add_paragraph("\n")

    document.add_paragraph("                表3 CO₂吸收传质系数测定数据处理表")
    table4 = document.add_table(len(param.data4[0]), 3, style='Table Grid')
    ...
    for i in range(len(param.data4)):
        for j in range(len(param.data4[0])):
            table4.rows[j].cells[1 + i].text = str(param.data4[i][j])
    document.add_paragraph('\n')
